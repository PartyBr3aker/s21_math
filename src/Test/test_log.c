...
START_TEST(log_test_1_rand) {
  ...
  if (x == 0.0) {
    ...
    ck_assert_ldouble_eq_tol(s21_log(x), log(x), ACCURACY);
  }
}
END_TEST
...
START_TEST(log_test_4_one) {
  ck_assert_ldouble_eq_tol(s21_log(1.0), log(1.0), ACCURACY);
...
Suite *suite_s21_log(void) {
  ...
  tcase_add_loop_test(tc, log_test_1_rand, 0, NUM_OF_TESTS);
