...
START_TEST(large) {
  ...
  ck_assert_ldouble_eq_tol(s21_fabs(a), fabs(a), ACCURACY);
}
END_TEST
...
START_TEST(fabs_loop_test_1) {
  ...
  if (rand() % 2) n = -n;
  ck_assert_ldouble_eq_tol(s21_fabs(n), fabs(n), ACCURACY);
}
END_TEST
...
Suite *suite_s21_fabs(void) {
  Suite *s = suite_create("float_modul");
  ...
  tcase_add_loop_test(tc, fabs_loop_test_1, 0, NUM_OF_TESTS);
