...
START_TEST(atan_test_1_rand) {
  ...
  ck_assert_ldouble_eq_tol(s21_atan(x), atan(x), ACCURACY);
}
END_TEST

START_TEST(atan_test_2_inf) {
  ck_assert_ldouble_eq_tol(s21_atan(INFINITY), (S21_PI_VALUE / 2.0), ACCURACY);
  ck_assert_ldouble_eq_tol(atan(INFINITY), (S21_PI_VALUE / 2.0), ACCURACY);
  ck_assert_ldouble_eq_tol(s21_atan((-INFINITY)), ((S21_PI_VALUE) / -2.0),
                           ACCURACY);
  ck_assert_ldouble_eq_tol(atan(-INFINITY), ((S21_PI_VALUE) / -2.0), ACCURACY);
}
END_TEST
...
START_TEST(atan_test_4_one) {
  ...
  ck_assert_ldouble_eq_tol(s21_atan(1.1), atan(1.1), ACCURACY);
  ck_assert_ldouble_eq_tol(s21_atan(-1.0), atan(-1.0), ACCURACY);
  ck_assert_ldouble_eq_tol(s21_atan(-1.1), atan(-1.1), ACCURACY);
}
END_TEST
...
Suite *suite_s21_atan(void) {
  ...
  tcase_add_loop_test(tc, atan_test_1_rand, 0, NUM_OF_TESTS / 10);
  tcase_add_loop_test(tc, atan_test_2_rand, 0, NUM_OF_TESTS / 10);
  tcase_add_loop_test(tc, atan_test_3_rand, 0, NUM_OF_TESTS / 10);
