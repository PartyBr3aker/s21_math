...
START_TEST(tan_test_1) {
  double x = RandomReal(-2 * S21_PI_VALUE, 2 * S21_PI_VALUE);
  ck_assert_ldouble_eq_tol(s21_tan(x), tanl(x), ACCURACY);
}
END_TEST

START_TEST(tan_0) {
  double x = 0;
  ck_assert_double_eq_tol(s21_tan(x), tanl(x), ACCURACY);
}
END_TEST
...
Suite *suite_s21_tan(void) {
  ...
  tcase_add_loop_test(tc, tan_test_1, 0, NUM_OF_TESTS);
